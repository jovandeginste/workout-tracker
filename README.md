...
For building the dev container locally, the variables `GO_VERSION` and `NODE_VERSION` must be set. Afterwards, the dev container can be built using the [Dev Container CLI](https://github.com/devcontainers/cli/) via `devcontainer build --workspace-folder ./.devcontainer-template/ --image-name ghcr.io/jovandeginste/workout-tracker-dev-container`. Here's a full example using PowerShell:
```powershell
