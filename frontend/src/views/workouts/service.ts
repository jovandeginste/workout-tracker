...
export class WorkoutService {
  ...
  private loadWorkout() {
    ...
    const preferredUnitsJSON = document.getElementById("workout-preferred-units").textContent;
  ...
  private updateDatapointInterval() {
    ...
    this.datapointInterval = intervals[Math.floor(intervals.length * 0.75)] / 1000 * 2;
  }
  ...
  public getIntervalStats(sIdx: number = 0, eIdx?: number): IntervalStats {
    ...
    const heartRateData = this._workoutData["heartRate"]?.data;
    if (heartRateData) {
      stats.heartRate = {
        avg: 0,
    ...
    for (let i = sIdx + 1; i <= eIdx; i++) {
      const duration = (timeData[i].getTime() - timeData[i - 1].getTime()) / 1000;
