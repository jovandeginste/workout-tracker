...
- Skip updates for non-GPX based workouts by @jovandeginste
...
- Add options to change the range and bucket size of statistics by @jovandeginste
...
- Add Italian to the list of automatically-updated translations by @jovandeginste
...
- Don't ask for location when adding workout without location by @jovandeginste
...
- Set Creator (source) when adding workout via web ui by @jovandeginste
...
- Don't show missing location when not location based by @jovandeginste
...
- Add more terms (with custom workouts) and the Dutch translations by @jovandeginste
...
- Fix disabling options in profile settings by @jovandeginste
...
- Add equipment (or gear) by @jovandeginste
...
- Fix the build time in Docker by @jovandeginste
...
- Convert leaflet controls & tooltip to dark scheme by @jovandeginste
...
- Improve the creation of a user profile by @jovandeginste
...
- Improve docs for Docker, add non-root explanation by @jovandeginste
...
- Add French language code by @jovandeginste
...
- Automatically use most popular workout in totals by @jovandeginste
...
- Replace chart.js with apexcharts by @jovandeginste
...
- Update npm packages by @jovandeginste
...
- Make the whole pill containing the username clickable by @jovandeginste
...
- Show the totals/records for the selected user by @jovandeginste
...
- Allow admins to manage some application-wide settings by @jovandeginste
...
- Add support for submitting workouts via API by @jovandeginste
...
- Add "Garmin" to the list of correct altitude creators by @jovandeginste
...
- Show the max average speed when showing records by @jovandeginste
...
- Ensure the workout-mapdata relation is a 1-1 mapping by @jovandeginste
...
- Calculate average of averages for statistics by @jovandeginste
...
- Prepare code for showing different statistic ranges by @jovandeginste
...
- Switch the timestamp detection in a GPX file by @jovandeginste
- Fix download of workouts by @jovandeginste in [#95](https://github.com/jovandeginste/workout-tracker/pull/95)
- Fix download of workouts by @jovandeginste
...
- Prevent duplicate MapData entries by @jovandeginste
...
- Update Human\* functions to return values without units by @jovandeginste
...
- Add per-user units to the user's profile by @jovandeginste
...
- Remove redundant workout statistics from workouts handlers and templates by @jovandeginste
...
- Convert user statistics to use the new function by @jovandeginste
...
- Switch per-workout records to use the new functions by @jovandeginste
- Show dashboard totals from new functions by @jovandeginste in [#79](https://github.com/jovandeginste/workout-tracker/pull/79)
- Show dashboard totals from new functions by @jovandeginste
...
- Add per-user setting to disable social sharing buttons by @jovandeginste
...
- Remove top-level data "currentUser", replace with CurrentUser function by @jovandeginste
...
- Build Docker image for ARM by @jovandeginste
...
- Create the profile for the initial admin user by @jovandeginste
...
- Pre-select the current type when editing a workout by @jovandeginste
...
- Add GPX details when refreshing a workout by @jovandeginste
...
- Remove top-level user profile language and totals show from templates by @jovandeginste
...
- Add (kind of) support for distance-less GPX and FIT by @jovandeginste
...
- Remove GPX when removing workout, and fix not null constraint by @jovandeginste
- Add FA icons for the new workout types by @jovandeginste in [#34](https://github.com/jovandeginste/workout-tracker/pull/34)
- Add FA icons for the new workout types by @jovandeginste
...
- Remove named return values by @jovandeginste
...
- Fix the initial minimum elevation by @jovandeginste
- Preload GPX when updating workouts by @jovandeginste
...
- Move original content of GPX file to separate table by @jovandeginste
...
- Use per-user timezone to show local times by @jovandeginste
...
- Translate the workout types in dropdown selection by @jovandeginste
- Add German translation by @jovandeginste in [#16](https://github.com/jovandeginste/workout-tracker/pull/16)
- Add German translation by @jovandeginste
...
- Return an error to the user when parsing fails by @jovandeginste
